...
function refreshIndexSheet() {
  ...
  const listArea = indexSheet.getRange(
    1,
    1,
    indexSheet.getMaxRows(),
...
function ensureIndexWeekDatePanel_(indexSheet: GoogleAppsScript.Spreadsheet.Sheet): void {
  ...
  const sheetNameRange = indexSheet.getRange(
    INDEX_WEEK_PANEL_START_ROW,
    INDEX_SHEET_NAME_COLUMN,
    maxRows - 1,
    1
  );
...
function extractDayAbbrFromSheetName_(sheetName: string): string {
...
function getDayOffsetFromMonday_(dayAbbr: string): number {
  ...
  return dayMap[dayAbbr.toUpperCase()] ?? 0;
}

function getMostRecentMonday_(today: Date): Date {
...
function enableInventoryAutoSyncTriggers() {
  ...
  spreadsheet.toast(
    `Inventory auto sync enabled (on open + every ${INVENTORY_AUTO_SYNC_INTERVAL_MINUTES} minutes).`,
    "Inventory Auto Sync",
    5
  );
}
...
function syncInventoryFromTrigger_(forceSync: boolean) {
  const now = Date.now();
...
function deleteInventoryAutoSyncTriggers_(): number {
  const triggers = ScriptApp.getProjectTriggers();
  let removedCount = 0;
  ...
  for (const trigger of triggers) {
    ...
    if (
      handlerFunction !== INVENTORY_AUTO_SYNC_TRIGGER_HANDLER
...
function handleIndexGoEdit_(
  ...
  if (!rangeIncludesColumn_(editedRange, INDEX_NAVIGATION_COLUMN)) return;
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    const targetSheet = spreadsheet.getSheetByName(targetSheetName);
    if (!targetSheet || !isNavigableLogSheetName_(targetSheetName)) continue;
...
function updateIndexDatesFromMondayCell_(indexSheet: GoogleAppsScript.Spreadsheet.Sheet): void {
...
interface AscExportRunResult {
  generatedCount: number;
  skippedDayTokens: string[];
  files: AscExportFileResult[];
  message: string;
}

interface AscExportDialogState {
  availableDayTokens: string[];
  suggestedDayToken: string;
...
function getAscExportDialogState(): AscExportDialogState {
  const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  const activeSheetName = spreadsheet.getActiveSheet().getName();
...
function exportAscForPreset(preset: string): AscExportRunResult {
...
function exportAscForRange(startDayToken: string, endDayToken: string): AscExportRunResult {
...
function exportAscForSpecificDays(dayTokens: string[]): AscExportRunResult {
  return exportAscForDayTokens_(dayTokens);
}

function exportAscForDayTokens_(dayTokens: string[]): AscExportRunResult {
  const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  ...
  const files: AscExportFileResult[] = [];
  const skippedDayTokens: string[] = [];
...
function buildAscExportResultMessage_(
  files: AscExportFileResult[],
  skippedDayTokens: string[]
): string {
  if (files.length === 0) {
    if (skippedDayTokens.length === 0) return "No ASC files were generated.";
  ...
  return (
    `Generated ${files.length} ASC file(s). ` +
    `Skipped: ${skippedDayTokens.join(", ")}.`
  );
}

function getAvailableAscDayTokens_(
  spreadsheet: GoogleAppsScript.Spreadsheet.Spreadsheet
): string[] {
  const dayTokens = new Set<string>();

  for (const sheet of spreadsheet.getSheets()) {
    if (!isLogSheetName_(sheet.getName())) continue;
...
function normalizeAscDayTokens_(dayTokens: string[]): string[] {
...
function normalizeAscDayToken_(dayToken: string): string {
  const normalized = String(dayToken ?? "").trim().toUpperCase();
  if (ASC_EXPORT_DAY_TOKENS.indexOf(normalized) < 0) return "";
  return normalized;
}

function getLogSheetDayToken_(sheetName: string): string {
  const dayToken = extractDayAbbrFromSheetName_(sheetName);
  if (ASC_EXPORT_DAY_TOKENS.indexOf(dayToken) < 0) return "";
  return dayToken;
}

function getAscExportSheetsForDay_(
  spreadsheet: GoogleAppsScript.Spreadsheet.Spreadsheet,
  dayToken: string
): GoogleAppsScript.Spreadsheet.Sheet[] {
  ...
  return spreadsheet
    ...
    .sort((left, right) => {
      ...
      if (mergeKeyDiff !== 0) return mergeKeyDiff;
...
function exportLegacySingleAscFromActiveSheet_() {
  ...
  const html = HtmlService.createHtmlOutput(
    "<style>body{font-family:Arial,sans-serif;padding:16px;margin:0;}" +
...
function formatTimeForAsc_(value: unknown, offsetSeconds = 0): string {
  ...
  if (value instanceof Date) {
    ...
    if (totalSeconds === null) {
      ...
      if (/^\d{1,2}:\d{2}:\d{2}$/.test(text)) return text.replace(/^(\d):/, "0$1:");
      if (/^\d{1,2}:\d{2}$/.test(text)) return text.replace(/^(\d):/, "0$1:") + ":00";
      return text;
    }
  }
...
function formatLengthForAsc_(value: unknown): string {
  const totalSeconds = parseLengthSeconds_(value);
  if (totalSeconds === null) return String(value ?? "").trim();
...
function normalizeAscLengthSeconds_(seconds: number): number {
  // Snap to the nearest of 30s or 60s
  return Math.abs(seconds - 30) <= Math.abs(seconds - 60) ? 30 : 60;
}

function escapeHtmlAttr_(value: string): string {
  return value
    .replace(/&/g, "&amp;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;")
